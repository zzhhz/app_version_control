...
JSONViewer = (function() {
				...
				if (value) {
					...
					if (lvl === 0) {
						...
						if (items.length) {
							...
							rootCount.addEventListener("click", function() {
								if (isMaxLvl) return;
						...
						spans.innerHTML = isArray ? "[" : "{";
						frag.appendChild(spans);
						frag.appendChild(rootCount);
					}
							...
							if (typeof item === "object") {
								...
								// array & object
									...
									if (!itemLen) {
										...
										itemsCount.addEventListener("click", itemLinkCb);
	...
	JSONViewer.prototype._createSimple = function(value) {
		let father = document.createElement("span");
		var spanEl = document.createElement("span");
		var type = typeof value;
		var txt = value;
		let prefixEl = document.createElement("span");
		let suffixEl = document.createElement("span");
		prefixEl.classList.add("type-string");
		suffixEl.classList.add("type-string");
		prefixEl.innerHTML = "\"";
		suffixEl.innerHTML = "\"";
		...
		father.appendChild(prefixEl);
		father.appendChild(spanEl);
		father.appendChild(suffixEl);
		return father;
	};
